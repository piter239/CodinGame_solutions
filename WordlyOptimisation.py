# Codingame puzzle https://www.codingame.com/puzzle/wordle
# look at the end of file for rules description
...
while True:
    state = list(
        map(int, input().split()))  # state: State of the letter of the corresponding position of previous guess

    for i in range(WORD_LENGTH):  # analyse the result of the previous guess
        if state[i] == 1:  # letter not in the word
            letter_not_in.add(guess[i])
            words_set -= contains_letter[guess[i]]
        elif state[i] == 2:  # letter in the word but at different position
            letter_known.add(guess[i])
            letter_not_at_postion[i].add(guess[i])
            words_set -= contains_letter_at_pos[(i, guess[i])]
            words_set &= contains_letter[guess[i]]
        elif state[i] == 3:  # letter it the word exactly at this position
            letter_known.add(guess[i])
    ...
    if len(words_set) < 5:  # just for test
        # choose the guess_s randomly
        guess_s = random.choice(list(words_set))
        p("Chosen", guess_s)
        for i, c in enumerate(guess_s):
            guess[i] = c
    else:
